...
import myhdl2
from myhdl2 import BlockError, BlockInstanceError, Cosimulation
from myhdl2._instance import _Instantiator
from myhdl2._util import _flatten
from myhdl2._extractHierarchy import (_makeMemInfo,
                                     _UserVerilogCode, _UserVhdlCode,
                                     _UserVerilogInstance, _UserVhdlInstance)
from myhdl2._Signal import _Signal, _isListOfSigs
...
class _bound_function_wrapper(object):
    ...
    def __init__(self, bound_func, srcfile, srcline):
        ...
        self.calls = 0
        # register the block
        myhdl2._simulator._blocks.append(self)
...
class _Block(object):
    ...
    def _inferInterface(self):
        from myhdl2.conversion._analyze import _analyzeTopFunc
    ...
    def  _clear(self):
        ...
        for b in myhdl2._simulator._blocks:
            b.calls = 0

    def verify_convert(self):
        self._clear()
        return myhdl2.conversion.verify(self)

    def analyze_convert(self):
        self._clear()
        return myhdl2.conversion.analyze(self)
    ...
    def convert(self, hdl='Verilog', **kwargs):
        ...
        if hdl.lower() == 'vhdl':
            converter = myhdl2.conversion._toVHDL.toVHDL
        elif hdl.lower() == 'verilog':
            converter = myhdl2.conversion._toVerilog.toVerilog
        else:
    ...
    def config_sim(self, trace=False, **kwargs) :
        ...
        if trace:
            for k, v in kwargs.items() :
                setattr(myhdl2.traceSignals, k, v)
            myhdl2.traceSignals(self)
    ...
    def run_sim(self, duration=None, quiet=0):
        if self.sim is None:
            ...
            self.sim = myhdl2._Simulation.Simulation(sim)
